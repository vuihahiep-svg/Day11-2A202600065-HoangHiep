...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "password_literal": r"(?i)password\s*[:=]\s*\S+",
...
async def llm_safety_check(response_text: str) -> dict:
    ...
    if first_line.startswith("UNSAFE") or "\nUNSAFE" in upper:
        is_safe = False
    elif "UNSAFE" in first_line:
        is_safe = False
    else:
        is_safe = "SAFE" in first_line or (
            "SAFE" in upper and "UNSAFE" not in first_line
        )
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        filt = content_filter(response_text)
        text_out = filt["redacted"]
        if filt["issues"]:
            self.redacted_count += 1

        if self.use_llm_judge:
            check = await llm_safety_check(text_out)
            if not check["safe"]:
                self.blocked_count += 1
                text_out = (
                    "I cannot provide that information. Please contact VinBank support "
                    "if you need help with your account."
                )

        safe_msg = types.Content(
            role="model",
            parts=[types.Part.from_text(text=text_out)],
        )
        if hasattr(llm_response, "content"):
            llm_response.content = safe_msg
        return llm_response
